...
def solve(input_text: str) -> tuple[int, int]:
    ...
    def iterate(cheat: bool, end_char: str = "E"):
        ...
        # Cheat timed out
        if cheat and char == "#" and cheat_active <= 0:
            return
        ...
        if cheat and cheat_used_at_pos != (-1, -1) and pos in came_from_score:
            paths.append((came_from_score[pos] + distance_to_start, cheat_used_at_pos))
            return
        ...
        # Already found a path without cheat, stop all attempt with a longer path
        if cheat and path_length_without_cheat < distance_to_start:
            return
        ...
        for neighbor in neighbors:
            ...
            # TODO Allow walk through wall if 1 <= cheat_active?
            # Activate cheat
            if cheat and cheat_used_at_pos == (-1, -1) and char == "#":
                heappush(
                    queue_cheat,
                    (distance_to_start + 1, *new_pos, 1, pos),
                )
    ...
    paths_that_save_100ps = sorted(
        [
            i
            for i in paths
            if i[0] <= path_length_without_cheat - 100 and i[1] is not None
        ]
    )

    answer_part1 = len(paths_that_save_100ps)
